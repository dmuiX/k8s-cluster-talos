...
OPTIONS:
    ...
    --skip-iso-download         Skip downloading Talos ISO image
...
EOF
    ...
    while read -r node_json; do
        ...
        export NODE_NAME=$(echo "$node_json" | jq -r '.name')
        export IP=$(echo "$node_json" | jq -r '.ip')
        export GATEWAY=$(echo "$node_json" | jq -r '.gateway')
        # Get MAC address from Terraform output (needed for hardware selector)
        export MAC=$(get_mac_for_node "$NODE_NAME")
        export VIRTUAL_IP=$(echo "$node_json" | jq -r '.virtual_ip')
        
        local patch_file="./node-configs/${NODE_NAME}-network-patch.yaml"
        debug "${NODE_NAME} network patch → ${patch_file}"
        ...
        # Apply patch
        talosctl machineconfig patch "$base_config" --patch @"$patch_file" --output "$NODE_NAME-patched.yaml"
...
get_ips_by_role() {
    yq e ".nodes[] | select(.role == \"$1\") | .ip"      "$NODES_FILE_PATH";
}

get_mac_for_node() {
    local name="$1"
    local hash=$(echo -n "$name" | md5sum | cut -c1-6)
    echo "52:54:00:${hash:0:2}:${hash:2:2}:${hash:4:2}"
}

get_names_by_role() {
    yq e ".nodes[] | select(.role == \"$1\") | .name"    "$NODES_FILE_PATH";
}

count_by_role(){
    yq e "[.nodes[] | select(.role == \"$1\")] | length" "$NODES_FILE_PATH";
}
...
credentials_prompt() {
    local required_vars="CLUSTER_NAME VMS_DIR CLUSTER_DIR NODES_FILE_PATH TALOS_ISO_URL VIRTUAL_IP CP_COUNT CP_BASE_IP GATEWAY NAMESERVERS"
    ...
    _ask VIRTUAL_IP               "Virtual IP for control plane"    ""
    _ask CP_COUNT                 "Number of control plane nodes"   "3"
    _ask CP_BASE_IP               "First control plane IP"          ""
    _ask GATEWAY                  "Gateway IP"                      ""
    _ask NAMESERVERS              "Nameservers (comma-separated)"   "8.8.8.8,1.1.1.1"
...
$(IFS=','; for ns in $NAMESERVERS; do echo "      - \"$ns\""; done)
    vcpus: ${CP_VCPUS:-4}
    memory_mib: ${CP_MEMORY_MIB:-7178}
    disk_size_gib: ${CP_DISK_GIB:-255}
    role: "control-node"
...
YAML
    ...
    success "Generated $file (${CP_COUNT} CP + ${WORKER_COUNT:-0} Worker)"
}
...
phase_create_vms() {
    if [ "$SKIP_TERRAFORM" = false ]; then
        ...
        export TF_VAR_nodes_file_path=$NODES_FILE_PATH
        ...
        CREATED_VMS=$($SUDO virsh list --all | grep -E "control-node|worker-node" | wc -l)
...
phase_bootstrap() {
    if [ "$SKIP_BOOTSTRAP" = false ]; then
        ...
        for NAME in $NAMES; do
            [ -z "$NAME" ] && continue
            CDROM=$(virsh domblklist "$NAME" --details | awk '$2=="cdrom" {print $3}')
            if [ -z "$CDROM" ]; then
                echo "⚠ ${NAME}: no CD-ROM device found, skipping"
                exit 1
            fi
            if virsh change-media "$NAME" --eject "$CDROM" --config &>/dev/null; then
                echo "✓ ${NAME}: ISO ejected from ${CDROM}, will boot from disk"
            else
                echo "⚠ ${NAME}: failed to eject ISO from ${CDROM}"
            fi
        done
...
main() {
    ...
    K8S_ENDPOINT="https://${VIRTUAL_IP}:6443"
